    ...
    def apply_transform(self, image, mask, transform):
        ...
        if transform_name in self.params and self.params[transform_name]:
            try:
                ...
                # Transform may not accept previously stored kwargs; call without them
...
class ApplyJPEGRoundtripCompression:
    """Apply a JPEG encode/decode roundtrip to simulate real-world compression artifacts.

    This runs as the final transform in the pipeline for both images and videos (per-frame).
    """

    def __init__(self, quality: int = 75):
        """
        Args:
            quality: explicit JPEG quality to use. Defaults to 75 to match requested behavior.
        """
        self.fixed_quality = int(quality)
        self.params = {}

    def __call__(self, img, mask=None):
        # Always apply with fixed quality (reused for paired frames)
        q = self.fixed_quality
        ...
        # Convert numpy array to PIL Image in a mode that preserves RGB channels
        try:
            ...
            buffer.seek(0)
            decoded_pil = Image.open(buffer)
            # Ensure we output RGB when input was RGB
            if pil_img.mode == "RGB":
                decoded_pil = decoded_pil.convert("RGB")
            out_img = np.array(decoded_pil)
        except Exception:
...
# Simple compression helpers (not part of transform pipelines)
